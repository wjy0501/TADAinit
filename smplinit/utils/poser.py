...
# 绘制人体骨架，接受一个image和pose，在image上绘制骨架
...
class Skeleton:
    def __init__(self, humansd_style=False, apose=False):
        ...
        if self.style == "humansd":
            # init pose [17, 3], in [-1, 1]^3  17个关节点初始三维坐标
            ...
            # init pose [18, 3], in [-1, 1]^3   18个关节点初始三维坐标
            self.points3D = np.array([
                [-0.00313026,  0.16587697,  0.05414092],
    ...
    # 从 SMPLX 网格表面采样 N 个点，并且return采样到的点
    ...
    # 沿着每条线段采样 N 个点，并在这些点上添加噪声。返回包含所有的采样点
    ...
    # 从json或预设中加载3D点数据,加载的数据存储到points3D中
    ...
    #  加载并处理 SMPL-X 模型，根据给定的参数生成三维人体网格和关键点，并进行一系列预处理
    def load_smplx(self, path, betas=None, expression=None, gender='neutral'):  # Path是smplx模型文件的路径
        import smplx, torch

        # 根据输入参数创建一个 SMPLX 模型实例
        ...
        # 初始化body_pose数组；if self.apose，调整特定关节的姿势参数
        ...
        # 获取并存储顶点和面片数据
        self.vertices = smplx_output.vertices.detach().cpu().numpy()[0]  # [10475, 3] 面片数10475
        self.faces = smplx_model.faces  # [20908, 3] 顶点数20908

        # 处理关节
        joints = smplx_output.joints.detach().cpu().numpy()[0]  # [127, 3]
        # 根据不同风格，不同的映射函数处理关节数据
        if self.style == "humansd":
